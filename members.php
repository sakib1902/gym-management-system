					...
					<div class="card-body">
						...
						<table class="table table-bordered table-condensed table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$member =  $conn->query("SELECT *,concat(firstname,' ',lastname,' ',middlename) as name from members order by concat(firstname,' ',lastname,' ',middlename) asc");
								...
								<tr>
									...
									<td class="text-center">
										...
										<button class="btn btn-sm btn-outline-primary edit_member" type="button" data-id="<?php echo $row['id'] ?>" >Edit</button>
										 <button class="btn btn-sm btn-outline-danger delete_member" type="button" onclick="delMember(<?php echo $row['id'] ?>)">Delete</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
